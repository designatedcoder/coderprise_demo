        ...
        <main class="w-full">
            <div class="sm:py-12">
                <div class="max-w-2xl bg-white dark:bg-gray-800 mx-auto mt-6 px-6 py-4 shadow-md overflow-hidden sm:rounded-lg sm:px-6 lg:px-8">
                    <form method="POST" action="{{ route('vehicles.update', $vehicle->uuid) }}" enctype="multipart/form-data" x-data='{
                        make: `{{ $vehicle->make }}`,
                        model: `{{ $vehicle->model }}`,
                        price: `{{ $vehicle->price }}`,
                        year: `{{ $vehicle->year }}`,
                        ...
                        <div class="flex flex-col space-y-3 sm:flex-row sm:space-y-0 sm:space-x-3 md:space-x-6">
                            <div class="sm:w-1/2">
                                ...
                                <x-text-input id="make" type="text" name="make" autofocus autocomplete="make" class="block mt-1 w-full" value="{{ old('make', $vehicle->make) }}" x-model="make" />
                                <x-input-error :messages="$errors->get('make')" />
                            </div>
                            <div class="sm:w-1/2">
                                <x-input-label for="model" :value="__('Model')" />
                                <x-text-input id="model" type="text" name="model" autofocus autocomplete="model" class="block mt-1 w-full" value="{{ old('model', $vehicle->model) }}" x-model="model" />
                        ...
                        <div class="flex flex-col space-y-3 mt-3 sm:flex-row sm:space-y-0 sm:space-x-3 md:space-x-6">
                            <div class="sm:w-1/2">
                                ...
                                <x-text-input id="price" type="number" name="price" autofocus autocomplete="price" class="block mt-1 w-full" value="{{ old('price', $vehicle->price) }}" x-model="price" />
                                <x-input-error :messages="$errors->get('price')" />
                            </div>
                            <div class="sm:w-1/2">
                                <x-input-label for="year" :value="__('Year')" />
                                <x-text-input id="year" type="number" name="year" autofocus autocomplete="year" class="block mt-1 w-full" value="{{ old('year', $vehicle->year) }}" x-model="year" />
                        ...
                        <div class="mt-3">
                            ...
                            <x-custom.text-area-input name="description" id="description" cols="30" rows="5" class="block mt-1 w-full" x-model="description" >{{ old('description', $vehicle->description) }}</x-custom.text-area-input>
                        ...
                        <div class="flex justify-between mt-6">
                            ...
                            <button type="submit" class="text-gray-50 bg-blue-600 px-4 py-2 rounded uppercase tracking-widest hover:bg-blue-500 disabled:opacity-50" :disabled="!make.length || !model.length || !price.length || !year.length || !description.length">
                                Edit Vehicle
                            </button>
                        </div>
                    </form>
